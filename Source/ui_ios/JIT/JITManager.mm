...
//  Priority: 1. Debugger → 2. DualMapping → 3. AltServer (passive check)
//
...
- (BOOL)initialize
{
	...
	// Priority 3: AltServer (passive check - already enabled externally)
	if([self isAltServerJITEnabled])
	{
		JIT_LOG_INFO("AltServer JIT detected");
	...
	[self setJITEnabled:NO withMethod:JITMethodNone status:@"No JIT available - attach debugger or use AltServer"];
	return NO;
}
...
{
	// Passive check: try to allocate RWX memory
	// If AltServer has already enabled JIT, this will succeed
	...
	{
		munmap(testMem, 4096);
		JIT_LOG_DEBUG("Direct RWX mapping succeeded - AltServer JIT likely enabled");
		return YES;
	}

	JIT_LOG_DEBUG("Direct RWX mapping failed - AltServer JIT not enabled");
	return NO;
}

- (void)tryEnableViaAltServer
{
	// AltServer JIT must be enabled externally via AltStore app
	// This is just a placeholder - the actual enabling happens in AltStore
	JIT_LOG_INFO("To enable JIT via AltServer, use AltStore to enable JIT for this app");
	
	_status = JITStatusPending;
	_statusDescription = @"Enable JIT via AltStore app";
